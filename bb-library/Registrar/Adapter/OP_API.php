...
class OP_API
{
 ...
 public function processRawReply (OP_Request $r) {
     ...
     $str = $this->_send($msg);
     if (!$str) {
         throw new Registrar_Exception("Bad reply", 4004);
     }
     if ($this->debug) {
        // var_dump($str);
         // echo "DEBUG:".$str . "\n";
 ...
 public function process (OP_Request $r) {
     ...
     protected static function parseArray ($node)
     {
         ...
         foreach ($node->childNodes as $child) {
             $name = self::decode($child->nodeName);
             if ('item' !== $name) {
                 throw new Registrar_Exception('Wrong message format', 4006);
             }
 ...
 class OP_Reply
 {
     ...
     protected function _parseReply ($str = '')
     {
         ...
         if ((!is_array($arr) && trim($arr) == '') ||
             $arr['reply']['code'] == 4005)
         {
             throw new Registrar_Exception("API is temporarily unavailable due to maintenance", 4005);
         }
